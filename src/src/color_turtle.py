...
class TurtleCamProcessor:
    def __init__(self):  # Inicialización de la clase
        ...
        if not self.cap.isOpened():
            rospy.logerr("Error: No se puede acceder a la cámara.")
            exit()

    def process_frame(self):
        ret, frame = self.cap.read()
        if not ret:
            rospy.logwarn("Error: No se puede capturar el frame.")
            return
        ...
        if largest_contour is not None:
            ...
            self.pub.publish(int(max_area))  # Publica el área detectada en el topic
