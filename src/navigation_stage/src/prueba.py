...
def button_callback(data, button_name, button_widgets):
    ...
    if data.data:  # Button is pressed
        if button_press_times[button_name] is None:  # First time the button is pressed
            button_press_times[button_name] = current_time
        # Update button color to show it's pressed
        button_widgets[button_name].config(bg='red')
    else:  # Button is released
        if button_press_times[button_name] is not None:  # Only if it was pressed before
...
def save_position(button_name):
    # Simulate retrieving and saving the robot's current position
    current_position = Pose()  # This should retrieve the robot's actual position
